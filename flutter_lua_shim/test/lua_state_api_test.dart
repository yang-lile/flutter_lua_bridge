...
void main() {
  ...
  group('LuaState - 辅助库 (luaL)', () {
    ...
    test('lRef 和 lUnref', () {
      ...
      // 创建一个引用表
      L.newTable();
      final refTableIdx = L.top; // 引用表在栈顶

      // 创建一个要引用的对象（表）
      L.newTable();
      final ref = L.lRef(refTableIdx);
      expect(ref, greaterThan(0));

      // 通过引用获取对象
      L.rawGetI(refTableIdx, ref);
      expect(L.isTable(-1), isTrue);
      L.pop(1); // 弹出获取的对象

      // 释放引用
      L.lUnref(refTableIdx, ref);
  ...
  group('LuaState - 版本信息', () {
    test('LUA_VERSION_RELEASE_NUM - 获取版本号', () {
      // LUA_VERSION_RELEASE_NUM 是一个整数，例如 50408 表示 5.4.7
      final versionNum = kLuaVersionReleaseNum;
      expect(versionNum, 50407); // 支持 5.4.7
      print('Lua version release number: $versionNum');

      // 验证版本号格式正确
      final major = versionNum ~/ 10000;
      final minor = (versionNum % 10000) ~/ 100;
      final release = versionNum % 100;
      expect(major, equals(5));
      expect(minor, 4);
      expect(release, 7);
      print('Lua version: $major.$minor.$release');
    });
  });
}
