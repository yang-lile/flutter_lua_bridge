...
Future<String> _downloadAndExtractLuaSource(
  ...
  try {
    ...
    if (response.statusCode != 200) {
      throw StateError(
        '[flutter_lua_shim] Failed to download Lua source: HTTP ${response.statusCode}',
      );
    }
    ...
    if (result.exitCode != 0) {
      throw StateError(
        '[flutter_lua_shim] Failed to extract Lua source: ${result.stderr}',
      );
    }
    
    // 查找解压后的目录（通常是 lua-x.y.z 格式）
    final extractedDirs = tempDir
        .listSync()
        .whereType<Directory>()
        .toList();
    ...
    if (extractedDirs.isEmpty) {
      throw StateError(
        '[flutter_lua_shim] No directory found after extraction',
      );
    }
...
/// 获取 Lua 版本的默认下载 URL
String _getDefaultLuaUrl(String version) {
  // 获取最新的补丁版本（例如 5.4.7）
  ...
  final fullVersion = versionMap[version] ?? version;
  return 'https://www.lua.org/ftp/lua-$fullVersion.tar.gz';
}
...
void main(List<String> args) async {
  await build(args, (BuildInput input, BuildOutputBuilder output) async {
    ...
    if (luaSourceUrl != null && luaSourceUrl.isNotEmpty) {
      // 从 URL 下载 Lua 源码
      print('[flutter_lua_shim] Downloading Lua source from: $luaSourceUrl');
      
      final cacheDir = p.join(pkgRoot, '.build_cache', 'lua', luaVersion);
      ...
      if (!Directory(luaSrcDir).existsSync()) {
        throw StateError(
          'Lua $luaVersion source not found at $luaSrcDir. '
          'Please provide lua_source_url in userDefines to download automatically.',
        );
      }
    }
